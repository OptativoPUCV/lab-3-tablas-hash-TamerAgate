...
{
    Pair ** anterior = map->buckets;
    long anteriorCapacidad = map->capacity;
    ...
    for (int i = 0; i < anteriorCapacidad; i++)
    {
        if (anterior[i] != NULL && anterior[i]->key != NULL) 
        {
            insertMap(map, anterior[i]->key, anterior[i]->value);
        }
    }
